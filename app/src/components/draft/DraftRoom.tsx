...
export default function DraftRoom({ data }: { data: DraftRoomData }) {
  ...
  const myPicks = myParticipant ? data.picks.filter((pick) => pick.league_member_id === myParticipant.league_member_id) : [];
    ...
    <main className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="mx-auto grid max-w-7xl gap-5 px-4 py-5 sm:px-6 lg:grid-cols-[240px_minmax(0,1fr)] xl:grid-cols-[240px_minmax(0,1fr)_280px]">
        ...
        <section id="available-teams" className="order-3 min-w-0 scroll-mt-4 lg:order-3" aria-labelledby="available-teams-heading"><div className="rounded-2xl border border-slate-800 bg-slate-900 p-4 shadow-lg"><div className="flex items-end justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-orange-400">Draft pool</p><h2 id="available-teams-heading" className="mt-1 text-2xl font-black">Available Teams</h2></div><span className="rounded-full bg-slate-800 px-3 py-1 text-xs font-black text-slate-300">{availableTeams.length} available</span></div><div className="mt-4 flex flex-col gap-3 sm:flex-row"><label className="min-w-0 flex-1"><span className="sr-only">Search available teams</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search FBS teams…" className="min-h-11 w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-2.5 outline-none transition focus:border-orange-500 focus:ring-1 focus:ring-orange-500" /></label><label><span className="sr-only">Filter by conference</span><select value={conference} onChange={(event) => setConference(event.target.value)} className="min-h-11 w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-2.5 outline-none focus:border-orange-500 sm:w-auto">{conferences.map((item) => <option key={item}>{item}</option>)}</select></label></div></div>
          <div className="mt-3 grid items-start gap-3 sm:grid-cols-2 2xl:grid-cols-3">{availableTeams.map((team) => { const intelligence = data.teamIntelligence[team.id]; return <article key={team.id} className={`${queuedTeamIds.has(team.id) ? "border-blue-500/50 bg-blue-950/35" : "border-slate-800 bg-slate-900"} rounded-xl border p-3.5 shadow-sm transition hover:-translate-y-0.5 hover:border-slate-600 hover:shadow-lg`}><div className="min-w-0"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="truncate text-lg font-black tracking-tight">{team.school_name}</p><p className="mt-0.5 truncate text-xs font-semibold text-slate-400">{team.conference} · {classificationLabel(intelligence?.classification ?? null)}</p></div>{intelligence?.apRank && <span className="shrink-0 rounded-full bg-orange-500/15 px-2 py-1 text-xs font-black text-orange-300 ring-1 ring-orange-400/20">AP #{intelligence.apRank}</span>}</div><div className="mt-3 flex items-center gap-2 text-xs"><span className="rounded-md bg-slate-800 px-2 py-1 font-mono font-black text-orange-300">{team.abbreviation}</span><span className="rounded-md bg-slate-950 px-2 py-1 font-bold text-slate-400">{intelligence?.priorSeason ?? "Prior"}: <strong className="text-slate-200">{recordLabel(intelligence?.priorRecord ?? null)}</strong></span>{queuedTeamIds.has(team.id) && <span className="ml-auto rounded-full bg-blue-500/15 px-2 py-1 font-black text-blue-300">In queue</span>}</div></div><details className="group mt-3 rounded-lg bg-slate-950/70"><summary className="cursor-pointer list-none px-2.5 py-2 text-xs font-bold text-blue-300 hover:text-blue-200 focus-visible:outline focus-visible:outline-2 focus-visible:outline-orange-400">Team details <span aria-hidden="true" className="ml-1 inline-block transition group-open:rotate-90">›</span></summary><dl className="grid grid-cols-2 gap-x-3 gap-y-2 border-t border-slate-800 px-2.5 py-2.5 text-xs"><div><dt className="text-slate-500">Conference</dt><dd className="mt-0.5 font-bold text-slate-200">{team.conference}</dd></div><div><dt className="text-slate-500">Classification</dt><dd className="mt-0.5 font-bold text-slate-200">{classificationLabel(intelligence?.classification ?? null)}</dd></div><div><dt className="text-slate-500">{intelligence?.priorSeason ?? "Prior season"} record</dt><dd className="mt-0.5 font-bold text-slate-200">{recordLabel(intelligence?.priorRecord ?? null)}</dd></div><div><dt className="text-slate-500">Current / preseason AP</dt><dd className="mt-0.5 font-bold text-slate-200">{intelligence?.apRank ? `#${intelligence.apRank}` : "Not ranked / unavailable"}</dd></div></dl></details><div className="mt-3 flex items-center gap-2"><button disabled={!mutationsEnabled || queuedTeamIds.has(team.id) || queuePending !== null} onClick={() => updateQueue(team.id, () => addQueueTeamAction(data.draft.id, team.id))} className="min-h-10 flex-1 rounded-lg border border-slate-600 px-3 py-2 text-xs font-black text-slate-200 transition hover:bg-slate-800 disabled:opacity-40">{queuedTeamIds.has(team.id) ? "✓ Queued" : "+ Add to queue"}</button><button disabled={!isMyTurn || pendingTeam !== null} onClick={() => draftTeam(team.id)} className="min-h-10 rounded-lg bg-orange-500 px-4 py-2 text-xs font-black text-slate-950 shadow-sm transition hover:bg-orange-400 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-400">{pendingTeam === team.id ? "Drafting…" : isMyTurn ? "Draft team" : "Draft"}</button></div></article>; })}</div>
