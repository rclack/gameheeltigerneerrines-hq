    ...
    <main className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="mx-auto grid max-w-7xl gap-5 px-4 py-5 lg:grid-cols-[240px_minmax(0,1fr)] xl:grid-cols-[240px_minmax(0,1fr)_280px]">
        ...
        <section className="order-3 min-w-0 lg:order-3"><div className="rounded-xl border border-slate-800 bg-slate-900 p-4"><div className="flex flex-col gap-3 sm:flex-row"><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search FBS teams…" className="min-w-0 flex-1 rounded-lg border border-slate-700 bg-slate-950 px-4 py-2.5 outline-none focus:border-orange-500" /><select value={conference} onChange={(event) => setConference(event.target.value)} className="rounded-lg border border-slate-700 bg-slate-950 px-4 py-2.5">{conferences.map((item) => <option key={item}>{item}</option>)}</select></div><p className="mt-2 text-sm text-slate-500">{availableTeams.length} available teams</p></div>
          <div className="mt-3 grid items-start gap-2 sm:grid-cols-2 2xl:grid-cols-3">{availableTeams.map((team) => { const intelligence = data.teamIntelligence[team.id]; return <article key={team.id} className="rounded-lg border border-slate-800 bg-slate-900 p-3 transition hover:border-slate-600"><div className="min-w-0"><div className="flex items-start justify-between gap-2"><p className="truncate text-base font-black">{team.school_name}</p>{intelligence?.apRank && <span className="shrink-0 rounded-full bg-orange-500/15 px-2 py-0.5 text-xs font-black text-orange-300">AP #{intelligence.apRank}</span>}</div><div className="mt-1 flex flex-wrap items-center gap-2 text-xs"><span className="rounded bg-slate-800 px-2 py-0.5 font-mono font-bold text-orange-300">{team.abbreviation}</span><span className="truncate text-slate-400">{team.conference}</span><span className="rounded border border-slate-700 px-1.5 py-0.5 font-semibold text-slate-300">{classificationLabel(intelligence?.classification ?? null)}</span></div></div><details className="group mt-2 rounded-md bg-slate-950/70"><summary className="cursor-pointer list-none px-2.5 py-2 text-xs font-bold text-blue-300 hover:text-blue-200 focus-visible:outline focus-visible:outline-2 focus-visible:outline-orange-400">Team details <span aria-hidden="true" className="ml-1 inline-block transition group-open:rotate-90">›</span></summary><dl className="grid grid-cols-2 gap-x-3 gap-y-2 border-t border-slate-800 px-2.5 py-2.5 text-xs"><div><dt className="text-slate-500">Conference</dt><dd className="mt-0.5 font-bold text-slate-200">{team.conference}</dd></div><div><dt className="text-slate-500">Classification</dt><dd className="mt-0.5 font-bold text-slate-200">{classificationLabel(intelligence?.classification ?? null)}</dd></div><div><dt className="text-slate-500">{intelligence?.priorSeason ?? "Prior season"} record</dt><dd className="mt-0.5 font-bold text-slate-200">{recordLabel(intelligence?.priorRecord ?? null)}</dd></div><div><dt className="text-slate-500">Current / preseason AP</dt><dd className="mt-0.5 font-bold text-slate-200">{intelligence?.apRank ? `#${intelligence.apRank}` : "Not ranked / unavailable"}</dd></div></dl></details><div className="mt-3 flex items-center gap-2"><button disabled={!mutationsEnabled || queuedTeamIds.has(team.id) || queuePending !== null} onClick={() => updateQueue(team.id, () => addQueueTeamAction(data.draft.id, team.id))} className="flex-1 rounded border border-slate-600 px-2 py-1.5 text-xs font-bold text-slate-200 hover:bg-slate-800 disabled:opacity-40">{queuedTeamIds.has(team.id) ? "Queued" : "+ Queue"}</button><button disabled={!isMyTurn || pendingTeam !== null} onClick={() => draftTeam(team.id)} className="rounded bg-orange-500 px-3 py-1.5 text-xs font-black text-slate-950 hover:bg-orange-400 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-400">{pendingTeam === team.id ? "…" : "Draft"}</button></div></article>; })}</div>
        </section>
