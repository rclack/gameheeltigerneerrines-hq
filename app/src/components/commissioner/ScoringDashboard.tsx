...
export default function ScoringDashboard({ league, rules, events, games, standings, draftedTeams, teams, cfbdConfiguration, syncRuns, livePollRuns, recapOperations }: Props) {
  ...
  return (
    <main className="min-h-screen bg-slate-100 text-slate-950 [&_.text-slate-500]:!text-slate-700 [&_.text-slate-600]:!text-slate-700 [&_input]:bg-white [&_input]:text-slate-950 [&_input::placeholder]:text-slate-500 [&_label]:text-slate-800 [&_select]:bg-white [&_select]:text-slate-950 [&_textarea]:bg-white [&_textarea]:text-slate-950">
      <header className="relative overflow-hidden bg-blue-950 text-white"><div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(249,115,22,0.22),transparent_38%)]" aria-hidden="true" /><div className="relative mx-auto flex max-w-7xl flex-col justify-between gap-4 px-4 py-7 sm:flex-row sm:items-end sm:px-6"><div><Link href={`/commissioner/${league.id}`} className="text-sm font-bold text-blue-200 hover:text-white">← Commissioner HQ</Link><p className="mt-4 text-xs font-black uppercase tracking-[0.18em] text-orange-300">Season operations</p><h1 className="mt-1 text-3xl font-black tracking-tight sm:text-4xl">Scoring Dashboard</h1><p className="mt-1 text-blue-100">{league.name} · {league.season}</p></div><div className="flex flex-col gap-2 sm:flex-row"><Link href={`/commissioner/${league.id}/captains`} className="rounded-lg border border-amber-300/40 bg-amber-400/15 px-4 py-2 text-center font-bold text-amber-100 hover:bg-amber-400/25">Captain Corrections</Link><Link href={`/league/${league.id}`} className="rounded-lg bg-white px-4 py-2 text-center font-bold text-blue-950 hover:bg-blue-100">League Home</Link><Link href={`/league/${league.id}/standings`} className="rounded-lg border border-white/20 bg-white/10 px-4 py-2 text-center font-bold hover:bg-white/20">View Standings</Link></div></div></header>
      <div className="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6">
        ...
        <section className="rounded-xl bg-white p-5 shadow sm:p-6">
          ...
          {livePollRuns.length ? <details className="mt-4"><summary className="cursor-pointer rounded-lg py-2 text-sm font-black text-blue-800">Live scoreboard telemetry ({livePollRuns.length})</summary><p className="mt-2 text-xs text-slate-600">Request attempts are our internal operational count. CFBD metered usage is the provider-reported quota sample and may follow different endpoint semantics.</p><div className="mt-2 overflow-x-auto"><table className="min-w-full text-left text-sm"><thead><tr className="border-b"><th className="p-2">Started</th><th className="p-2">Status</th><th className="p-2">Request Attempts</th><th className="p-2">Provider Games</th><th className="p-2">Canonical</th><th className="p-2">Drafted</th><th className="p-2">Drafted Live</th><th className="p-2">Changed</th><th className="p-2">Unchanged</th><th className="p-2">CFBD Metered Usage</th></tr></thead><tbody>{livePollRuns.map((poll)=><tr className="border-b" key={poll.id}><td className="p-2 whitespace-nowrap">{formatTimestamp(poll.started_at)}</td><td className="p-2 font-bold">{poll.status}</td><td className="p-2">{poll.provider_calls} <span className="whitespace-nowrap text-xs text-slate-500">({poll.scoreboard_calls} scoreboard + {poll.info_calls} info)</span></td><td className="p-2">{poll.provider_game_count}</td><td className="p-2">{poll.canonical_game_count}</td><td className="p-2">{poll.drafted_relevant_game_count}</td><td className="p-2">{poll.drafted_live_game_count}</td><td className="p-2">{poll.changed_game_count}</td><td className="p-2">{poll.unchanged_game_count}</td><td className="p-2">{poll.quota_used ?? '—'} / {poll.quota_monthly_limit ?? '—'}</td></tr>)}</tbody></table></div></details> : <p className="mt-4 rounded-lg bg-slate-50 p-4 text-slate-700">No live scoreboard polls have run. Recurring polling remains inactive.</p>}
        </section>
        ...
        <section className="grid gap-6 xl:grid-cols-2">
          <div className="rounded-xl bg-white p-6 shadow">
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              ...
              <label className="text-sm font-semibold">Week (optional)<input type="number" min={0} value={manual.week} onChange={(event) => setManual({ ...manual, week: event.target.value })} placeholder="Season-level if blank" className="mt-1 w-full rounded-lg border p-2" /></label>
          ...
          <div ref={gameFormRef} className={`scroll-mt-6 rounded-xl bg-white p-6 shadow ${game.gameId ? "ring-2 ring-orange-500" : ""}`}>
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              <label className="text-sm font-semibold">Week<input type="number" min={0} value={game.week} onChange={(event) => setGame({ ...game, week: Number(event.target.value) })} className="mt-1 w-full rounded-lg border p-2" /></label>
