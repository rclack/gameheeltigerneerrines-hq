...
export default async function MyScorePage({ params }: { params: Promise<{ leagueId: string }> }) {
  ...
  const activeEvents = teams.reduce((sum, team) => sum + team.events.length, 0);
      ...
      <div className="mx-auto max-w-5xl space-y-5 px-4 py-6 sm:px-6 sm:py-8">
        ...
        {teams.length ? teams.map((teamScore) => (
          <details key={teamScore.team.id} className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm open:shadow-md">
            ...
            <div className="space-y-5 border-t px-4 pb-5 pt-5 sm:px-5">{teamScore.events.length ? [...new Set(teamScore.events.map((event) => event.week))].sort((a, b) => (a ?? 999) - (b ?? 999)).map((week) => <div key={week ?? "season"}><h4 className="mb-2 text-sm font-black uppercase tracking-wide text-slate-500">{week ? `Week ${week}` : "Season"}</h4>{teamScore.events.filter((event) => event.week === week).map((event) => <div key={event.id} className="flex justify-between gap-4 border-b py-2 last:border-0"><div className="min-w-0"><p className="font-semibold">{event.rule.display_name}</p>{event.notes && <p className="text-sm text-slate-500">{event.notes}</p>}</div><span className={`${event.points > 0 ? "text-green-700" : event.points < 0 ? "text-red-700" : "text-slate-600"} shrink-0 font-black`}>{event.points > 0 ? "+" : ""}{event.points}</span></div>)}</div>) : <p className="text-slate-500">No scoring events yet.</p>}</div>
          </details>
