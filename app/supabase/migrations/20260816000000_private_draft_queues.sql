-- Private, per-owner draft queues.
...
create table public.draft_queue_items (
  ...
  constraint draft_queue_items_member_team_key unique (draft_id, league_member_id, team_id),
  constraint draft_queue_items_member_position_key
    unique (draft_id, league_member_id, queue_position) deferrable initially immediate
);

create index draft_queue_items_member_idx on public.draft_queue_items (league_member_id);
create index draft_queue_items_team_idx on public.draft_queue_items (draft_id, team_id);

create trigger draft_queue_items_set_updated_at
before update on public.draft_queue_items
for each row execute function public.set_updated_at();

alter table public.draft_queue_items enable row level security;

-- Queue preferences are private. Commissioner status does not grant access.
create policy "Users can read only their own draft queue"
on public.draft_queue_items for select to authenticated
using (
  exists (
...
create function public.add_team_to_my_draft_queue(target_draft_id uuid, target_team_id uuid)
...
create function public.remove_team_from_my_draft_queue(target_queue_item_id uuid)
returns boolean
language plpgsql
security definer
set search_path = ''
...
create function public.move_team_in_my_draft_queue(target_queue_item_id uuid, move_direction integer)
returns boolean
language plpgsql
security definer
set search_path = ''
...
-- Runs inside submit_draft_pick's transaction. All owners lose the drafted team
-- and each affected queue is compacted to consecutive preference positions.
create function public.remove_drafted_team_from_queues()
returns trigger
language plpgsql
security definer
set search_path = ''
...
create trigger draft_pick_cleans_private_queues
...
alter publication supabase_realtime add table public.draft_queue_items;
