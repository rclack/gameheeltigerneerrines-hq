...
test("forward repair preserves the RPC contract and returns after persisting a post-wait lock", async () => {
  const sql = (await readFile(new URL("../../supabase/migrations/20260827132106_persist_weekly_lineup_kickoff_locks.sql", import.meta.url), "utf8")).toLowerCase();
